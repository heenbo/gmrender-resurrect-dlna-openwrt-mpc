...
static void output_mpc_set_uri(const char *uri,
...
printf("set_uri:%s\n", uri);
	mpd_run_clear(conn);
	mpd_run_add(conn, gsuri_);
//	mpd_run_add(conn, "http://www.aistoy.com:9331/story/qizhi/古希腊神话/太阳神大战恶龙.mp3");
	mpd_run_play(conn);
#endif
...
static int output_mpc_play(output_transition_cb_t callback) {
...
static int output_mpc_pause(void) {
...
static int output_mpc_init(void)
{
...
struct output_module mpc_output = {
        .shortname = "mpc",
	.description = "GStreamer multimedia framework",
	.init        = output_mpc_init,
//	.add_options = output_mpc_add_options,
	.set_uri     = output_mpc_set_uri,
//	.set_next_uri= output_mpc_set_next_uri,
	.play        = output_mpc_play,
//	.stop        = output_mpc_stop,
	.pause       = output_mpc_pause,
//	.seek        = output_mpc_seek,
